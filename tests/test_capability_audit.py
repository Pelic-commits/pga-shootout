...
def test_owned_state_comes_from_current_sqlite_and_is_partitioned():
    report = audit()
    assert report.owned_clubs == 80
    assert report.owned_occurrences == 146
    assert (
    ...
    assert report.owned_fully_simulated_clubs == 36
    assert report.owned_clubs_with_unresolved_ability == 44
